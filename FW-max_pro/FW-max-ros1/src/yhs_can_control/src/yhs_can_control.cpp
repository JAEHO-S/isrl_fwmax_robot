...
namespace yhs_tool
{
  void CanControl::cmd_velCallBack(const geometry_msgs::Twist::ConstPtr& msg)
  {
    yhs_can_msgs::ctrl_cmd converted;
    converted.ctrl_cmd_gear = 6; // 기본 주행 기어
    converted.ctrl_cmd_x_linear = msg->linear.x;
    converted.ctrl_cmd_y_linear = 0.0; // 필요시 설정
    converted.ctrl_cmd_z_angular = msg->angular.z * 180.0 / 3.14;

    ctrl_cmdCallBack(converted); // 기존 함수 재사용
  }
  ...
  void CanControl::run()
  {
    ...
    cmd_vel_sub_ = nh_.subscribe<geometry_msgs::Twist>("cmd_vel", 5, &CanControl::cmd_velCallBack, this); // 추가
